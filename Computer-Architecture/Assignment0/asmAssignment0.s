...
do_Str:                        		; do_Str function definition - functions are defined as labels
	...
	yourCode:			; use label to build a loop for treating the input string characters
                ...
                cmp byte [ecx], ' '     ; check if the current char is a " "
                je incCounter           ; if it is, jump to the counter increase section
                cmp byte [ecx], 'A'     ; compare current char with 'A'
                jl nextLetter           ; if smaller than 'A' jump to the next letter
                cmp byte [ecx], 'Z'     ; compare current char with 'Z'
                jg nextLetter           ; if larger than 'Z' jump to the next letter
                add dword [ecx], 0x20   ; add 32 to the current letter (to change into its lower counterpart)
                jmp nextLetter          ; jump to the next letter

                incCounter:
                        inc dword [an]  ; increase the space-counter an by 1
