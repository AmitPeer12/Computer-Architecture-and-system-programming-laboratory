...
Elf32_Shdr *sectionAdrByNum(int secNum)
{
    return (void *)((int)header + header->e_shoff + calculateOffset(secNum));
}
void *addressByOff(int offset)
{
    return (void *)((int)header + offset);
}
int calculateOffset(int secNum)
{
    return header->e_shentsize * secNum;
}
char *getSectionNameByNDX(int ndx)
{
    return &sectionStringTable[ndx];
}

void printRel(Elf32_Shdr *relSec)
{
    Elf32_Rel *rel = (Elf32_Rel *)addressByOff(relSec->sh_offset);
    Elf32_Shdr *symbolTableSection = sectionAdrByNum(relSec->sh_link); //Dynsym
    Elf32_Shdr *symbolTable = addressByOff(symbolTableSection->sh_offset);
    Elf32_Sym *symbol;
    char *symbolStringTable = (char *)(addressByOff(sectionAdrByNum(symbolTableSection->sh_link)->sh_offset)); //from Dynstr
    printf("Relocation section %s at offset %X contains %d entries:\n",
           getSectionNameByNDX(relSec->sh_name),
           relSec->sh_offset,
           (relSec->sh_size) / (relSec->sh_entsize));
    printf("%-10s %-10s %-10s %-10s %-10s\n", "Offset", "Info", "Type", "Sym.Value", "Sym. Name");
    for (int i = 0; i < (relSec->sh_size) / (relSec->sh_entsize); i++)
    {
        symbol = (Elf32_Sym *)((int)symbolTable + (ELF32_R_SYM(rel->r_info) * symbolTableSection->sh_entsize));
        if (dMode == 1)
            printf("R_SYM %d ", ELF32_R_SYM(rel->r_info));
        printf("%-10X %-10X %-10u %-10X %-10s\n",
               rel->r_offset,
               rel->r_info,
               ELF32_R_TYPE(rel->r_info),
               symbol->st_value,
               &symbolStringTable[symbol->st_name]);
...
void relocationTable()
{
    ...
    int secNum = 0;
    Elf32_Shdr *secP = sectionAdrByNum(secNum);
    for (int i = 0; i < header->e_shnum; i++)
    {
        if (secP->sh_type == SHT_REL)
            printRel(secP);
        secNum++;
        secP = sectionAdrByNum(secNum);
    }
}